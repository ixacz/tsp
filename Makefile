...
EXE_NAME_LINUX_DEBUG=tsp-linux-debug 
EXE_NAME_LINUX_RELEASE=tsp-linux-release

EXE_NAME_WINDOWS=tsp-windows
EXE_NAME_WINDOWS_DEBUG=tsp-windows-debug 
EXE_NAME_WINDOWS_RELEASE=tsp-windows-release
...
ZIP_INPUT_FILES= $(BINDIR) $(SRC_INPUT_FILE) $(TSP_FILES) $(OPT_TOUR_FILES) LICENSE README.md 
...
build-linux-debug: $(SRC_INPUT_FILE) | $(BINDIR)
	@echo "Building Debugging executable $(EXE_NAME_LINUX_DEBUG)..."
	@$(CC_LINUX) $(SRC_INPUT_FILE) $(CFLAGS) $(CDEBUGFLAGS) -o $(BINDIR)/$(EXE_NAME_LINUX_DEBUG)

build-linux-release: $(SRC_INPUT_FILE) | $(BINDIR)
	@echo "Building Release executable $(EXE_NAME_LINUX_RELEASE)..."
	@$(CC_LINUX) $(SRC_INPUT_FILE) $(CFLAGS) $(CRELEASEFLAGS) -o $(BINDIR)/$(EXE_NAME_LINUX_RELEASE)

$(EXE_NAME_WINDOWS): build-windows-debug build-windows-release

build-windows-debug: $(SRC_INPUT_FILE) | $(BINDIR)
	@echo "Building Debugging executable $(EXE_NAME_WINDOWS_DEBUG)..."
	@$(CC_WINDOWS) $(SRC_INPUT_FILE) $(CFLAGS) $(CDEBUGFLAGS) -o $(BINDIR)/$(EXE_NAME_WINDOWS_DEBUG)

build-windows-release: $(SRC_INPUT_FILE) | $(BINDIR)
	@echo "Building Release executable $(EXE_NAME_WINDOWS_RELEASE)..."
	@$(CC_WINDOWS) $(SRC_INPUT_FILE) $(CFLAGS) $(CRELEASEFLAGS) -o $(BINDIR)/$(EXE_NAME_WINDOWS_RELEASE)
...
build-dist: $(ZIP_OUTPUT_FILE) 

$(ZIP_OUTPUT_FILE):
	@echo "Zipping the distribution..."
	@zip -r $(ZIP_OUTPUT_FILE) $(ZIP_INPUT_FILES)
