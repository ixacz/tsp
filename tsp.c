...
#define __ROW(i, j) ((i) < (j) ? (i) : (j))
#define __COL(i, j) ((i) > (j) ? (i) : (j))
#define GET_EDGE_INDEX(i, j, n) ((((i) == (j) ? -1 : (__ROW(i, j) * (n) - (__ROW(i, j) * (__ROW(i, j) + 1) / 2) + (__COL(i, j) - __ROW(i, j) - 1)))))

int get_edge_index(int i, int j, int n) {
    if (i == j) return -1;
    int row = (i < j) ? i : j;
    int col = (i > j) ? i : j;
...
void init_graph_ctx(GraphContext* ctx, int n) {
    ...
    ctx->distances = (int*) malloc(ctx->edge_count * sizeof(int));
    ctx->penalized_costs = (double*) malloc(ctx->edge_count * sizeof(double));
    ...
    ctx->working_1_tree = malloc(ctx->node_count * sizeof(int));
    ctx->best_1_tree = malloc(ctx->node_count * sizeof(int));
}
...
int calculate_euclidean_distance(GraphContext* ctx, int fromid, int toid) {
    long long dx = ctx->nodes[fromid].x - ctx->nodes[toid].x;
    long long dy = ctx->nodes[fromid].y - ctx->nodes[toid].y;
...
void init_edges(GraphContext* ctx) {
    for (int i = 0; i < ctx->node_count - 1; i++) {
        int fromid = ctx->nodes[i].id;
        for (int j = i + 1; j < ctx->node_count; j++) {
            int toid = ctx->nodes[j].id;
            int dist = calculate_euclidean_distance(ctx, fromid, toid);

            int edge_idx = GET_EDGE_INDEX(i, j, ctx->node_count);

            ctx->edges[edge_idx].id = edge_idx;
            ctx->edges[edge_idx].from = fromid;
            ctx->edges[edge_idx].to = toid;

            ctx->distances[edge_idx] = dist;
            ctx->penalized_costs[edge_idx] = (double) dist;
        }
    }
}
...
void apply_edges_penalized_cost(GraphContext* ctx) {
    for (int i = 0; i < ctx->edge_count; i++) {
        ...
        double dist = ctx->distances[i];
        ctx->penalized_costs[i] = dist + pi1 + pi2;
    }
}
...
void _calculate_node_degrees(GraphContext* ctx, int t1, int t2) {
    for (int i = 0; i < ctx->node_count; i++) ctx->degrees[i] = 0;

    for (int i = 0; i < ctx->node_count; i++) {
        int parent = ctx->working_1_tree[i];
        if (parent != -1) {
            ctx->degrees[i]++;
            ctx->degrees[parent]++;
        }
...
void _calculate_1_tree_weight(GraphContext* ctx, int t1, int t2) {
    double total_penalized_weight = 0.0;

    for (int i = 0; i < ctx->node_count; i++) {
        int parent = ctx->working_1_tree[i];
        if (parent != -1) {
            int edge_idx = GET_EDGE_INDEX(i, parent, ctx->node_count);
            total_penalized_weight += ctx->penalized_costs[edge_idx];
        }
    }

    int idx1 = GET_EDGE_INDEX(0, t1, ctx->node_count);
    int idx2 = GET_EDGE_INDEX(0, t2, ctx->node_count);

    total_penalized_weight += ctx->penalized_costs[idx1];
    total_penalized_weight += ctx->penalized_costs[idx2];

    // Compute the standard Held-Karp lower bound adjustment: L(pi) = W(T_pi) - 2 * sum(pi_i).
    ctx->curr_lower_bound = total_penalized_weight - 2 * _sum_penalized_values(ctx);
}

void extract_1_tree(GraphContext* ctx) {
    int n = ctx->node_count;
    
    // Update Pi values for all edges.
    apply_edges_penalized_cost(ctx);

    // Stores the cheapest cost to connect a node to the current tree.
    double* min_dist_to_tree = malloc(n * sizeof(double));
    bool* visited = calloc(n, sizeof(bool));

    for (int i = 0; i < n; i++) {
        min_dist_to_tree[i] = DBL_MAX;
        ctx->working_1_tree[i] = -1;
    }

    // Start at node 1.
    min_dist_to_tree[1] = 0.0;
    
    for (int count = 0; count < n - 1; count++) {
        // find the lowest weight unvisited node connecting to our current node.
        int u = -1;
        double min_val = DBL_MAX;

        for (int i = 1; i < n; i++) {
            // if a node is not visisted and its weight is less than u's.
            if (!visited[i] && min_dist_to_tree[i] < min_val) {
                min_val = min_dist_to_tree[i];
                u = i;
            }
        }
        ...
        // Update te distance of neighbors.
        for (int v = 1; v < n; v++) {
            if (u != v && !visited[v]) {
                int edge_idx = get_edge_index(u, v, n);
                double penalized_cost = ctx->penalized_costs[edge_idx];

                if (penalized_cost < min_dist_to_tree[v]) {
                    min_dist_to_tree[v] = penalized_cost;
                    ctx->working_1_tree[v] = u;
                }
            }
        }
    }
    ...
    int target1 = -2, target2 = -1;

    for (int i = 1; i < n; i++) {
        int edge_idx = get_edge_index(0, i, n);
        double penalized_costs = ctx->penalized_costs[edge_idx];

        if (penalized_costs < min1) {
            min2 = min1; target2 = target2;
            min1 = penalized_costs; target1 = i;
        } else if (penalized_costs < min2) {
            min2 = penalized_costs; target2 = i;
        }
    }

    ctx->working_1_tree[target1] = 0;
    ctx->working_1_tree[target2] = 0;
    ctx->working_1_tree[0] = -1;

    _calculate_node_degrees(ctx, target1, target2);
    _calculate_1_tree_weight(ctx, target1, target2);

    free(min_dist_to_tree);
    free(visited);
}

bool validate_1_tree(GraphContext* ctx) {
    int n = ctx->node_count;
    int* degrees = calloc(n, sizeof(int));
    bool valid = true;

    for (int i = 1; i < n; i++) {
        if (ctx->working_1_tree[i] == -1 && i != 1) {
            printf("CRITICAL ERROR: Node %d is disconnected!.\n", i);
            valid = false;
        } else {
            degrees[i]++;
            degrees[ctx->working_1_tree[i]]++;
        }   
    }

    if (degrees[0] != 2) {
        printf("CRITICAL ERROR: Node 0 degree is %d (Must be 2)!.\n", degrees[0]);
        valid = false;    
    }
...
void print_1_tree(GraphContext* ctx) {
    int mst_nodes = 0;
    int nodes_connected_to_root = 0;

    int n = ctx->node_count;
    printf("1-Tree Connections: (Node -> Root Path):\n");
    ...
    for (int i = 0; i < n; i++) {
        ...
        while (curr != -1) {
            printf("%d", curr);
            curr = ctx->working_1_tree[curr];
            if (curr != -1) {
                printf(" -> ");
                if (curr == 1) {
                    nodes_connected_to_root++;
                }
            } else {
                mst_nodes++;
            }
    ...
    printf("MST Tree Nodes: %d\n, Nodes connected to root: %d\n", mst_nodes, nodes_connected_to_root);
}
...
void estimate_target_bound(GraphContext* ctx) {
    ...
    for (int step = 1; step < n; step++) {
        ...
        for (int i = 0; i < n; i++) {
            if (!visited[i]) {
                double dist = ctx->distances[GET_EDGE_INDEX(current_node, i, n)];
                if (dist < min_dist) {
                    min_dist = dist;
                    next_node = i;
                }
    ...
    ctx->upper_bound = ctx->distances[GET_EDGE_INDEX(current_node, ctx->upper_bound_tour[0], n)];
    free(visited);
...
int main()
{
    ...
    // // Calculate Inital Upper Bound.
    // ctx.upper_bound = estimate_target_bound(&ctx);
    //
    // // Run Held-Karp Relaxation loop to maximize the Lower Bound of 1-Tree.
    // optimaize_held_karp_relaxation(&ctx);
    //
